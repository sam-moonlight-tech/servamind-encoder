export const TERABYTE = 1024 ** 4;
export const GIGABYTE = 1024 ** 3;
export const MEGABYTE = 1024 ** 2;
export const KILOBYTE = 1024;

export const COMPRESSED_FILE_TYPE = "serva";
export const MAX_FILE_SIZE = 10 * GIGABYTE;

export const EXTERNAL_LINKS = {
  COMPANY: "https://www.servamind.com",
  TERMS: "https://www.servamind.com/terms/",
  PRIVACY: "https://www.servamind.com/privacy/",
  DOCUMENTATION: "https://www.servamind.com",
  CONTACT: "https://www.servamind.com/contact/",
} as const;
